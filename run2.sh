...
while IFS=' ' read -r number molecule; do
    ...
    # run simulation
    CUDA_VISIBLE_DEVICES=0 python simulate.py --config_yml configs/simulate/spice_dt.yml --model_dir MODELPATH/maceoff_split_gemnet_dT_100k --init_idx $number
    ...
    if [ "$line_count1" -gt 500 ]; then
        CUDA_VISIBLE_DEVICES=0 python simulate.py --config_yml configs/simulate/spice_t.yml --model_dir MODELPATH/maceoff_split_gemnet_T_100k --init_idx $number
...
done < md_scripts/num_atoms_leq_15_copy.txt
